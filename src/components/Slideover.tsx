...
import { Registration, PaymentStatusType, RegistrationStatusType, TicketTypeType } from "@/lib/schemas"
...
export const Slideover = ({ isOpen, onClose, onApprove, onReject, onUpdate, isLoading, registration }: SlideoverProps) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (paymentData) {
          ...
          setEditedRegistration(prev => prev ? { ...prev, payment: paymentData } : null);
        }
      }
    } catch (error) {
  ...
  const handlePaymentRequest = async () => {
    ...
    try {
      const checkResponse = await fetch(`/api/payments/${editedRegistration.id}`);
      if (checkResponse.ok) {
        const existingPayment = await checkResponse.json();
        if (existingPayment) {
          setEditedRegistration(prev => prev ? { ...prev, payment: existingPayment } : null);
          toast({
            title: "Existing Payment Found",
            description: "A payment request already exists for this registration.",
            type: "background",
            variant: "destructive",
          });
          return;
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to handle payment request",
        variant: "destructive",
      });
    }
  };
  ...
  const handleEditPayment = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to update payment request",
        variant: "destructive",
      });
    }
  };
  ...
  console.log('Current editedRegistration:', editedRegistration);
              ...
              >
                <Dialog.Panel className="pointer-events-auto w-screen max-w-2xl">
                  <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl dark:bg-slate-900">
                    <div className="px-4 sm:px-6 py-6 bg-gray-50 dark:bg-slate-800">
                      ...
                      <div className="space-y-6">
                        <Card>
                          ...
                          <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 ">
                              {[
                                { label: "Registration Type", name: "registrationType" as keyof Registration },
                                { label: "Company", name: "company" as keyof Registration },
                                { label: "Designation", name: "designation" as keyof Registration },
                                { label: "City", name: "city" as keyof Registration },
                                { label: "Phone Number", name: "phoneNumber" as keyof Registration },
                                { label: "Created At", name: "createdAt" as keyof Registration, readOnly: true },
                              ].map((field) => (
                                <div key={field.name} className="space-y-2">
                                  <label className="text-sm font-medium text-gray-500 dark:text-gray-400">{field.label}</label>
                                  {isEditing && !field.readOnly ? (
                                    field.name === "registrationType" ? (
                                      <Select
                                        name={field.name}
                                        value={editedRegistration?.[field.name] as string}
                                      ...
                                      >
                                        ...
                                        <SelectContent>
                                          {["SPONSOR", "SPEAKER", "MEDIA", "VISITOR", "OTHERS"].map((type) => (
                                    ...
                                    <p className="text-sm text-gray-900 dark:text-gray-100">
                                      {field.name === "createdAt" 
                                        ? formatDate(registration[field.name] as Date) 
                        ...
                        <Card>
                          <CardHeader>
                            ...
                            <CardDescription>Manage payment status and ticket type</CardDescription>
                          </CardHeader>
                          <CardContent>
                              ...
                              {editedRegistration?.payment ? (
                                <>
                                  <div>
                                    ...
                                    <p className="text-sm text-gray-900 dark:text-gray-100">{editedRegistration.payment.status}</p>
                                  </div>
                                  <div>
                                    <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Ticket Type</label>
                                    <p className="text-sm text-gray-900 dark:text-gray-100">{editedRegistration.payment.ticketType}</p>
                                  </div>
                                  <div>
                                    <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Amount</label>
                                    <p className="text-sm text-gray-900 dark:text-gray-100">{editedRegistration.payment.amount} {editedRegistration.payment.currency}</p>
                                  </div>
                                  <Select
                                  ...
                                  >
                                    ...
                                    <SelectContent>
                                      {["ONE_DAY", "TWO_DAY", "FULL", "FREE", "VVIP", "VIP", "PASS"].map((type) => (
                                        <SelectItem key={type} value={type}>{type.replace('_', ' ')}</SelectItem>
                                      ))}
                                    </SelectContent>
                                  </Select>
                                  <Button onClick={handleEditPayment} disabled={!selectedTicketType}>
                                ...
                                <div className="space-y-4">
                                  ...
                                  <Button 
                                    ...
                                    disabled={!selectedTicketType || editedRegistration?.status !== 'APPROVED'}
                                  >
                                    <CreditCard className="mr-2 h-4 w-4" />
                                    {editedRegistration?.payment ? 'View Payment Request' : 'Create Payment Request'}
                                  </Button>
                                </div>
                              )}
                            </div>
                        ...
                        <Card>
                          <CardHeader>
                            ...
                            <CardDescription>Add or view comments about this registration</CardDescription>
                          </CardHeader>
                          <CardContent>
                            <div className="space-y-4">
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button 
                              onClick={async () => {
                                if (editedRegistration) {
                                  const updatedRegistration = await onApprove(editedRegistration.id);
                                  setEditedRegistration(updatedRegistration);
                                }
                              }} 
                              variant="default"
                              disabled={isLoading || registration.status === 'APPROVED'}
                          ...
                          <TooltipContent>
                            {registration.status === 'APPROVED' ? 'Registration already approved' : 'Approve this registration'}
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button 
                              onClick={async () => {
                                if (editedRegistration) {
                                  const updatedRegistration = await onReject(editedRegistration.id);
                                  setEditedRegistration(updatedRegistration);
                                }
                              }} 
                              variant="destructive"
                              disabled={isLoading || registration.status === 'REJECTED'}
                            >
                              {isLoading ? (
                                <span className="flex items-center">
                                  <span className="animate-spin mr-2 h-4 w-4 border-2 border-white border-t-transparent rounded-full"></span>
                          ...
                          <TooltipContent>
                            {registration.status === 'REJECTED' ? 'Registration already rejected' : 'Reject this registration'}
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </div>
